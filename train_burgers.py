...
def build_model(res) -> (Encoder1D, PointWiseDecoder1D,hyena1d):
...
def get_arguments(parser):
    ...
    parser.add_argument(
        '--resume', default='False', help='If set resumes training from provided checkpoint. (default: None)'
    )
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    encoder, decoder,hyena = build_model(res)
    
    if use_cuda:
        encoder = encoder.to(device)
        decoder = decoder.to(device)
        hyena = hyena.to(device)
    if use_cuda:
    ...
    hyena_optim = torch.optim.Adam(list(hyena.parameters()), lr=opt.lr, weight_decay=1e-8)
    ...
    with tqdm(total=opt.iters) as pbar:
        ...
        while True:
            ...
            if use_cuda:
                ...
                #x, y = x.to(f'cuda:{encoder.device_ids[0]}'),y.to(f'cuda:{encoder.device_ids[0]}')
            #print(x.shape)
            #print(y.shape)
            ...
            input_pos = prop_pos = gridx.repeat([x.shape[0], 1, 1])#.to(f'cuda:{encoder.device_ids[0]}')
        # x, input_pos = pad_pbc(x, input_pos, dx)
            ...
            if (n_iter-1) % opt.ckpt_every == 0 or n_iter >= opt.iters:
                ...
                make_image_grid(all_in_seq, all_pred, all_gt,
                                os.path.join(sample_dir, f'result_iter:{opt.resolution}_{n_iter}_{j}.png'))
                ...
                np.save("test_val_{}_{}.npy".format(opt.resolution,seed),np.mean(all_avg_loss)*1e4)
                ...
                ckpt = {
                    ...
                    'hyena_bottleneck': hyena.state_dict(),
                    'n_iter': n_iter,
                ...
                torch.save(ckpt,"./burgers_{}_{}_{}.pt".format(opt.resolution,n_iter,seed))
