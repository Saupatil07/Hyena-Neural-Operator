...
from loss_fn import rel_loss, rel_l2norm_loss,HsLoss
#from utils import load_checkpoint, save_checkpoint, ensure_dir
import torch.nn.functional as F
#from Sophia import DecoupledSophia,SophiaG
...
for seed in range(0,3):
    ...
    def build_model(opt) -> (SpatialTemporalEncoder2D, PointWiseDecoder2D,hyena1d):
    ...
    if __name__ == '__main__':
        ...
        if opt.path_to_resume != 'none':
            enc_optim = torch.optim.AdamW(list(encoder.parameters()), lr=opt.lr, weight_decay=1e-8)
            dec_optim = torch.optim.AdamW(list(decoder.parameters()), lr=opt.lr, weight_decay=1e-8)
            hyena_optim = torch.optim.AdamW(list(hyena_bottleneck.parameters()), lr=opt.lr, weight_decay=1e-8)
            ...
            ckpt = torch.load(opt.path_to_resume)  # custom method for loading last checkpoint
            ...
            #                            final_div_factor=1e4,
            #                            )
            # hyena_scheduler = OneCycleLR(hyena_optim, max_lr=opt.lr, total_steps=opt.iters,
            #                            div_factor=1e4,
            #                            final_div_factor=1e4,
            #                            )
        ...
        with tqdm(total=opt.iters) as pbar:
            ...
            while True and (not opt.eval_mode):
                ...
                if opt.eval_mode or ((n_iter-1) % opt.ckpt_every == 0 or n_iter >= opt.iters):
                    ...
                    np.save("test_val_{}_{}.npy".format("1e5_10000",seed),np.mean(all_avg_loss)*1e4)
                    ...
                    if not opt.eval_mode:
                        ...
                        torch.save(ckpt,"/home/ssp2/Desktop/HNO/comp_hyena/TRIAL_2/model_2/OFormer/final_results_navier/1e4/latent_hyena_{}_{}_{}.pt".format(seed,opt.save_name,n_iter))
                        del ckpt
